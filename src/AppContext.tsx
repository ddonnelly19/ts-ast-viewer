...
import { compilerVersionCollection, getCompilerApi, hasLoadedCompilerApi, type ScriptKind, type ScriptTarget } from "./compiler/index.js";
...
export function AppContextProvider({ children }: { children: React.ReactNode }) {
	...
	}, [state.currentFile, state.files[state.currentFile], state.options.scriptTarget, state.options.compilerPackageName, state.options.bindingEnabled]);
...
function setBindingGlobals(globals: { checker?: unknown; program?: unknown; type?: unknown; symbol?: unknown; signature?: unknown }) {
	const windowAny = window as any;
...
async function buildTsgoSourceFile(packageName: TsgoPackageName, files: Record<string, string>, currentFile: string): Promise<PrebuiltSourceFile> {
