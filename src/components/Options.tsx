...
export function Options(props: OptionsProps) {
	...
	function getEnumOption<T extends Record<string, string | number>, V extends string | number | readonly string[]>(
		name: string,
		prefix: string,
		e: T,
		currentValue: V,
		...
		const selection = (
			<select value={currentValue} onChange={(event) => onChange(parseInt(event.target.value, 10) as any)}>
