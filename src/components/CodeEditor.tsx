...
export class CodeEditor extends React.Component<CodeEditorProps, CodeEditorState> {
	...
	private createEditor(monaco: Monaco) {
		...
		this.disposables.push(
			editor.onMouseDown((e: monacoEditorForTypes.editor.IEditorMouseEvent) => {
	...
	private updateMonacoCompilerOptions() {
		...
		const { typescriptDefaults } = this.monaco.typescript;
		const compilerOptions = typescriptDefaults.getCompilerOptions() ?? {};
		if (this.props.scriptTarget != null) {
			compilerOptions.target = this.props.scriptTarget as never;
			typescriptDefaults.setCompilerOptions(compilerOptions);
		}
	}

	private updateModelLanguage() {
		if (this.monaco == null || this.editor == null) {
			return;
		}
		...
		const scriptKindEnum = (this.monaco.languages as { typescript?: { ScriptKind?: { TS?: number; JS?: number } } }).typescript?.ScriptKind;
		const scriptKind = this.props.scriptKind ?? scriptKindEnum?.TS ?? 0;
		const language = scriptKind === scriptKindEnum?.JS ? "javascript" : "typescript";
	...
	private updateHighlight() {
		...
		const range =
			highlight == null ? undefined : (
				(() => {
					const startInfo = this.lineAndColumnComputer.getNumberAndColumnFromPos(highlight.start);
					const endInfo = this.lineAndColumnComputer.getNumberAndColumnFromPos(highlight.end);
					return {
		...
		if (range != null) {
			try {
				this.editor.revealRangeInCenterIfOutsideViewport(range as any);
			} catch {
